...
'''
Fetch the max scores from the test pickle file
'''
...
'''
Utility functions to compute probabilities
'''
def compute_xent(x):
...
'''
Compute xent and estimate exponential dist params
'''
...
'''
Plot the histrograms of xent vs density histogram
'''
plt.subplot(3,1,1)
...
plt.xlim([0, 2.0])
plt.xlabel('negative log-max-confidence')
plt.ylabel('PDF')
plt.legend(['estimated exponential distribution', 'density histogram'])
plt.title('P( negative log-max-confidence | correct classification)')
plt.grid()
...
plt.ylabel('PDF')
plt.legend(['estimated exponential distribution', 'density histogram'])
plt.title('P( negative log-max-confidence | incorrect classification)')
plt.grid()
...
plt.ylabel('PDF')
plt.legend(['estimated exponential distribution', 'density histogram'])
plt.title('P( negative log-max-confidence ) (marginal)')
plt.grid()
...
plt.xlabel('max-log-confidence')
plt.ylabel('PDF')
plt.legend(['P(correct | negative log-max-confidence)', 'P(incorrect | negative log-max-confidence)'])
plt.title('Posterior distribution of classifications given negative log-max-confidence values')
plt.grid()

plt.show()

'''
compute the confusion matrix of rejected samples (classified as misclassified)
'''
